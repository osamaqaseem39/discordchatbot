...
class ProductResearchCommands(commands.Cog):
    ...
    async def _process_ai_request(self, ctx, prompt, title, command_type="general"):
        ...
        if predefined_key:
            ...
            if predefined:
                # For predefined responses, don't limit the content length
                content = predefined['content']
                
                # Split into multiple embeds if content is too long for Discord (4096 char limit)
                if len(content) > 4096:
                    chunks = [content[i:i+4096] for i in range(0, len(content), 4096)]
                    
                    for i, chunk in enumerate(chunks):
                        embed = discord.Embed(
                            title=f"{predefined['title']} (Part {i+1}/{len(chunks)})",
                            description=chunk,
                            color=BOT_CONFIG['embed_color']
                        )
                        if i == len(chunks) - 1:  # Only add footer to last chunk
    ...
    async with message.channel.typing():
        try:
                ...
                if predefined:
                    ...
                    if len(content) > 4096:
                        ...
                        for i, chunk in enumerate(chunks):
                            ...
                            if i == len(chunks) - 1:  # Only add footer to last chunk
                                embed.set_footer(text="💡 Quick response - No API usage • Use specific commands for detailed guidance")
            ...
            if len(response) > BOT_CONFIG['max_response_length']:
                ...
                for i, chunk in enumerate(chunks):
                    embed = discord.Embed(
                        title=f"🤖 AI Assistant (Part {i+1}/{len(chunks)})",
                        description=chunk,
                        color=BOT_CONFIG['embed_color']
                    )
                    if i == len(chunks) - 1:  # Only add footer to last chunk
